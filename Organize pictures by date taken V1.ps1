...
$dir = Get-Location
cd $dir
$filemetadata = Get-FileMetaData $dir
$i = 0
$fileFormat

$fileName = $filemetadata | Select-Object -ExpandProperty "Name"
$dateTaken = $filemetadata | Select-Object -ExpandProperty "Date taken"

#Changes date format so that it can be made into dir in windows
For($i=0;$i -lt $dateTaken.length;$i++){   
    $dateTaken[$i] = $dateTaken[$i].Substring(0,12).Replace("/",".")
}
...
For($i=0;$i -lt $dateTaken.length;$i++){   
    ...
    #Generates dir names
    $filePath[$i] = ".\" + $dateTaken[$i]

    #Write-Host $filePath[$i]

    #Tests for File path if it exists then it doesnt duplicate it
        If (!(Test-Path $filePath[$i])){                  
            #Write-Host $filePath[$i]
            Write-Host "Making Dir" $filePath[$i]
            mkdir $filePath[$i]
            Move-Item -Path $fileName[$i] -Destination $filePath[$i]
        }else{
            Write-Host $filePath[$i] "Exist Already"
            Move-Item -Path $fileName[$i] -Destination $filePath[$i]
        }
}